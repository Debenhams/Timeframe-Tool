 ...
 * Manages the new home dashboard widgets.
 */
(function(APP) {
    const Dashboard = {};
    ...
    Dashboard.initialize = () => {
        // Cache the content areas of our new widgets
        ELS.dailyStats = document.querySelector("#widget-daily-stats .widget-content");
        ELS.rotationAlerts = document.querySelector("#widget-rotation-alerts .widget-content");
        ELS.unassigned = document.querySelector("#widget-unassigned .widget-content");
    };

    // The main render function for the dashboard
    Dashboard.render = async () => {
        if (!ELS.dailyStats) return; // Haven't been initialized yet
        ...
        if (!weekStart) {
            ELS.dailyStats.innerHTML = "<p>Please select a week.</p>";
            return;
        }

        // Show loading messages while we fetch data
        setLoadingMessages();
    ...
    const setLoadingMessages = () => {
        if (ELS.dailyStats) ELS.dailyStats.innerHTML = "<p>Loading stats...</p>";
        if (ELS.rotationAlerts) ELS.rotationAlerts.innerHTML = "<p>Loading alerts...</p>";
        if (ELS.unassigned) ELS.unassigned.innerHTML = "<p>Loading advisor list...</p>";
    };
    ...
    const renderDailyStats = (allExceptions, weekStart) => {
        ...
        // Find all exceptions that fall within this week
        ...
        if (ELS.dailyStats) {
            ELS.dailyStats.innerHTML = `
                <h3 style="font-size: 28px; margin: 0; color: var(--brand-primary);">${exceptionsThisWeek.length}</h3>
                <p>Total Live Exceptions</p>
            `;
        }
    };
    ...
    const renderRotationAlerts = (effectiveMap, allAdvisors, weekStart) => {
        ...
        effectiveMap.forEach((assignment, advisorId) => {
            // Find assignments that *end* this week
        ...
        if (ELS.rotationAlerts) {
            if (alerts.length === 0) {
                ELS.rotationAlerts.innerHTML = "<p>No rotations are ending this week.</p>";
                return;
            }
            let html = '<ul style="margin: 0; padding-left: 20px; color: var(--color-text);">';
            alerts.forEach(alert => {
                html += `<li style="margin-bottom: 8px;"><strong>${alert.name}</strong> (ends ${alert.endDate})</li>`;
            });
            html += '</ul>';
            ELS.rotationAlerts.innerHTML = html;
        }
    };

    // 3. Unassigned Advisors Widget
    const renderUnassigned = (effectiveMap, allAdvisors) => {
        const unassigned = allAdvisors.filter(adv => {
            // Find any advisor who is *not* in the list of effective assignments
            return !effectiveMap.has(adv.id);
        });

        if (ELS.unassigned) {
            if (unassigned.length === 0) {
                ELS.unassigned.innerHTML = "<p>All advisors have an assignment for this week.</p>";
                return;
            }
            let html = '<ul style="margin: 0; padding-left: 20px; color: var(--color-text);">';
            unassigned.forEach(adv => {
                html += `<li style="margin-bottom: 8px;"><strong>${adv.name}</strong></li>`;
            });
            html += '</ul>';
            ELS.unassigned.innerHTML = html;
        }
    };
